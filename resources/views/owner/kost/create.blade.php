...
@section('title', 'Tambah Kost - KostMudah')
    ...
    <div class="modal-overlay">
        ...
        <div class="modal-container">
            ...
            <div
                ...
                <h2 class="text-[#001220] text-xl font-semibold">Tambah Properti Baru</h2>
            ...
            <form action="{{ route('owner.kost.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <textarea name="address" required rows="3"
                        ...
                        placeholder="Masukkan alamat lengkap kost">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="kelurahan" required
                            ...
                            <option value="Tebet" {{ old('kelurahan') == 'Tebet' ? 'selected' : '' }}>Tebet</option>
                            <option value="Menteng" {{ old('kelurahan') == 'Menteng' ? 'selected' : '' }}>Menteng</option>
                            <option value="Kuningan" {{ old('kelurahan') == 'Kuningan' ? 'selected' : '' }}>Kuningan
                            </option>
                            <option value="Mulyosari" {{ old('kelurahan') == 'Mulyosari' ? 'selected' : '' }}>Mulyosari
                            </option>
                            <option value="Senopati" {{ old('kelurahan') == 'Senopati' ? 'selected' : '' }}>Senopati
                            </option>
                            <option value="Grogol" {{ old('kelurahan') == 'Grogol' ? 'selected' : '' }}>Grogol</option>
                            <option value="Kemanggisan" {{ old('kelurahan') == 'Kemanggisan' ? 'selected' : '' }}>
                                Kemanggisan</option>
                        </select>
                        @error('kelurahan')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="flex gap-6 pt-1">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="type" value="putra"
                                    {{ old('type') == 'putra' ? 'checked' : '' }}
                                    class="w-5 h-5 border-[#73777D] text-[#06283D] focus:ring-[#06283D]">
                                <span class="text-[#191C1D] text-base">Putra</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="type" value="putri"
                                    {{ old('type') == 'putri' ? 'checked' : '' }}
                                    class="w-5 h-5 border-[#73777D] text-[#06283D] focus:ring-[#06283D]">
                                <span class="text-[#191C1D] text-base">Putri</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="type" value="campur"
                                    {{ old('type') == 'campur' ? 'checked' : '' }}
                                    class="w-5 h-5 border-[#73777D] text-[#06283D] focus:ring-[#06283D]">
                                <span class="text-[#191C1D] text-base">Campur</span>
                            </label>
                        </div>
                        @error('type')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price_per_month" value="{{ old('price_per_month') }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="total_rooms" value="{{ old('total_rooms', 1) }}" required
                            class="w-full px-4 py-3 bg-white border border-[#C3C7CD] rounded-lg input-focus text-[#191C1D]"
                            placeholder="1" min="1">
                        @error('total_rooms')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="rules" rows="2"
                        class="w-full px-4 py-3 bg-white border border-[#C3C7CD] rounded-lg input-focus text-[#191C1D] resize-none"
                        placeholder="Contoh: Jam malam 22.00">{{ old('rules') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <p class="text-[#73777D] text-xs mt-1">* Minimal upload 1 foto</p>
                    @error('photos.*')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            files.forEach((file, index) => {
                ...
                reader.onload = function(e) {
                    ...
                    container.innerHTML = `
                        ...
                        <button type="button" onclick="removePhoto(this, ${index})" 
