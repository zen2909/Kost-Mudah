    ...
    <div class="flex justify-between items-center px-6 py-4">
        ...
        <div class="flex items-center gap-3">
            <div class="text-right">
                <p class="text-[#191C1D] text-sm font-bold">{{ Auth::user()->name ?? 'Tenant' }}</p>
                @php
                    $user = Auth::user();
                    $role = $user->roles->first();
                    $roleName = $role ? strtoupper($role->name) : 'TENANT';
                @endphp
            ...
            <div
                ...
                {{ Auth::user()->photo ? '' : 'bg-gradient-to-r from-green-500 to-teal-500' }}">
                @if (Auth::user()->photo)
                    <img src="{{ Storage::url(Auth::user()->photo) }}" alt="{{ Auth::user()->name }}"
                        class="w-full h-full object-cover">
                @else
                    {{ Str::upper(substr(Auth::user()->name ?? 'TN', 0, 2)) }}
                @endif
            </div>
        </div>
    </div>
